...
class InventoryControllerIntegrationTest {
  ...
  void reserve() throws Exception {
    ...
    jdbcTemplate.update(
        "INSERT INTO product_products (id, name, description, price_amount, category_id) VALUES (?, ?, ?, ?, ?)",
        productId, "Test Product 2", "Desc", 2000, categoryId);

    // versionカラムも含めて初期化 (0)
    jdbcTemplate.update(
        "INSERT INTO inventory_inventories (product_id, quantity, version) VALUES (?, ?, ?)", productId,
        100, 0);

    // 実行 & 検証
    mockMvc
        .perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post("/api/inventory/reservations")
    ...
    Integer quantity = jdbcTemplate.queryForObject(
        "SELECT quantity FROM inventory_inventories WHERE product_id = ?", Integer.class, productId);
    Long version = jdbcTemplate.queryForObject(
        "SELECT version FROM inventory_inventories WHERE product_id = ?", Long.class, productId);
  ...
  void reserveInsufficientStock() throws Exception {
    ...
    jdbcTemplate.update(
        "INSERT INTO product_products (id, name, description, price_amount, category_id) VALUES (?, ?, ?, ?, ?)",
        productId, "Test Product 3", "Desc", 3000, categoryId);
    jdbcTemplate.update(
        "INSERT INTO inventory_inventories (product_id, quantity, version) VALUES (?, ?, ?)", productId,
        5, 0);
  ...
  void reserveOptimisticLockError() throws Exception {
    ...
    jdbcTemplate.update(
        "INSERT INTO product_products (id, name, description, price_amount, category_id) VALUES (?, ?, ?, ?, ?)",
        productId, "Test Product 4", "Desc", 4000, categoryId);
    jdbcTemplate.update(
        "INSERT INTO inventory_inventories (product_id, quantity, version) VALUES (?, ?, ?)", productId,
        100, 0);
    ...
    doAnswer(invocation -> {
      ...
      java.util.Optional<Inventory> result = (java.util.Optional<Inventory>) invocation.callRealMethod();

      // DB更新 (versionを1に進める)
      jdbcTemplate.update("UPDATE inventory_inventories SET version = version + 1 WHERE product_id = ?", productId);

      return result;
    }).when(inventoryRepository).findByProductId(new ProductId(productId));
